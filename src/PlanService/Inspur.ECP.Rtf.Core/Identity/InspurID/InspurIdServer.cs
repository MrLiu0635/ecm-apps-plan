...
{
    public class InspurIdServer : IIdentityServer
    {
        ...
        public async Task<EcpState> InspurIdSSO(string auth)
        {
            ...
            EcpUserService userSvr = new EcpUserService();
            SysUser user = userSvr.GetUserByInspurID(ar.ID);
            // SysUser user = new SysUser() { ID = ar.ID, Code = ar.Code, Mail = ar.Mail, Name = ar.first_name + ar.last_name,Phone = ar.Phone};
            if (user == null)
            {
                // 没有域账户的人员
                user = new SysUser() { ID = ar.ID, Code = ar.Code, Mail = ar.Mail, Name = ar.first_name + ar.last_name, Phone = ar.Phone };
            }
            EcpState state = new EcpState() { User = user, TenantID = ar.Enterprise.ID, TenantName = ar.Enterprise.Name };
            return state;
        }
